...
int main(int argc, char* argv[]){
	...
	while(right-left > 1){
		...
		int pid1 = fork();
		if(pid1){
			//insert a wait and addition/comparison logic here
			close(returnPipe[1]);
			retValLeft = (ReturnVals*)malloc(sizeof(ReturnVals));
			read(returnPipe[0], retValLeft, sizeof(ReturnVals));
			wait(pid1);
			close(returnPipe[0]);
		}else{
			right = (right+left)/2;
			close(returnPipe[0]);
			parentAccess = returnPipe[1];
			continue;
		}
		...
		if(pid2){
			...
			read(returnPipe[0], retValRight, sizeof(ReturnVals));
			wait(pid2);
			close(returnPipe[0]);
		}else{
			left = (right+left)/2 + 1;
